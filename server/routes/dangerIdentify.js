...
router.post('/add', permission('dangerIdentify:create'), async function (ctx, next) {
  const body = ctx.request.body
  const err = validateRequired(body, ['name', 'detail'])
  if (err) { ctx.body = new ErrorModel(err); return }
...
router.post('/update', permission('dangerIdentify:update'), async function (ctx, next) {
  const body = ctx.request.body
  const err = validateId(body.id) || validateRequired(body, ['name', 'detail'])
  if (err) { ctx.body = new ErrorModel(err); return }
  const repeat = await repeatName(body)
...
router.post('/del', permission('dangerIdentify:delete'), async function (ctx, next) {
