...
export function setToken(data: DataInfo<Date>) {
  ...
  function setUserKey(username: string, roles: Array<string>, loginTime: string, department: string) {
  ...
  if (data.username && data.roles) {
    const { username, roles, loginTime,department } = data;
    setUserKey(username, roles, loginTime,department);
  } else {
    const username =
      storageLocal().getItem<DataInfo<number>>(userKey)?.username ?? "";
    const roles =
      storageLocal().getItem<DataInfo<number>>(userKey)?.roles ?? [];
    setUserKey(username, roles, loginTime);
  }
}
