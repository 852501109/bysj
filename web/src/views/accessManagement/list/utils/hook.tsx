...
export function useObj(tableRef: Ref, treeRef: Ref) {
  ...
  function handleDelete(row) {
    delAccessManagement({id: row.id}).then(() => {
  ...
  const initExcel = (data) => {
    ...
    writeFile(workBook, "外来人员管理.xlsx");
  };
  ...
  async function onSearch() {
    ...
    console.log(param)
    const { data } = await getAccessManagementList(param);
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(valid => {
          if (valid) {
            ...
            if (title === "新增") {
              // 实际开发先调用新增接口，再进行下面操作
              addAccessManagement(curData).then((res) => {
                if(res.errno === 0) {
                  chores();
                } else {
                  message(res.message, {
              ...
              // 实际开发先调用修改接口，再进行下面操作
              updateAccessManagementList(curData).then((res) => {
                if(res.errno === 0) {
                  chores();
                } else {
                  message(res.message, {
