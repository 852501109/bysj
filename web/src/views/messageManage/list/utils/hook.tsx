...
export function useObj(tableRef: Ref, treeRef: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      cellRenderer: ({ row, props }) => (
        <el-tag
          size='medium'
          type={row.approvalStatus === '已解决' ? "success" : 'warning'}
          effect="plain"
        >
          {row.approvalStatus === '已解决' ? "已解决" : row.approvalStatus}
        </el-tag>
      )
    },
  ...
  async function onSearch() {
    loading.value = true;
    const param = Object.assign(toRaw(pagination), toRaw(form), { department: useUserStoreHook().department})
    console.log(param)
    ...
    addDialog({
      ...
      props: {
        formInline: {
          ...
          name:row?.name ?? '',
          content: row?.content ?? "",
          approvalDepartment: row?.approvalDepartment ?? "",
          approvalStatus: "等待批示",
        }
      },
      width: "46%",
