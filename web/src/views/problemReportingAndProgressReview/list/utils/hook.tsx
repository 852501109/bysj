...
import {
  ...
  addProblemReportingAndProgressReview,
  updateProblemReportingAndProgressReviewList,
...
export function useObj(tableRef: Ref, treeRef: Ref) {
  ...
  const columns: TableColumnList = [
    ...
    },
    {
      ...
      cellRenderer: ({row}) => {
        return (
          <div class="w-full">{row.approvalStatus}</div>)
      }
    },
  ...
  function handleDelete(row) {
    delProblemReportingAndProgressReview({id: row.id}).then(() => {
  ...
  async function onSearch() {
    loading.value = true;
    const param = Object.assign(toRaw(pagination), toRaw(form), {username: useUserStoreHook()?.username, roles: useUserStoreHook()?.roles.join(',')})
    const { data } = await getProblemReportingAndProgressReviewList(param);
    ...
    addDialog({
      ...
      props: {
        formInline: {
          ...
          name:row?.name ?? '',
          content: row?.content ?? "",
          approvalDepartment: row?.approvalDepartment ?? "等待问题分配",
          approvalStatus: row?.approvalStatus ?? "等待批示",
      ...
      beforeSure: (done, { options }) => {
        ...
        FormRef.validate(valid => {
          if (valid) {
            ...
            if (title === "新增") {
              // 实际开发先调用新增接口，再进行下面操作
              addProblemReportingAndProgressReview(curData).then((res) => {
                if(res.errno === 0) {
                  chores();
                } else {
                  message(res.message, {
              ...
              // 实际开发先调用修改接口，再进行下面操作
              updateProblemReportingAndProgressReviewList(curData).then((res) => {
                if(res.errno === 0) {
                  chores();
                } else {
                  message(res.message, {
